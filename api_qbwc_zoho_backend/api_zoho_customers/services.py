...
def load_customers_from_main_load(headers, params, username, pc_ip):
    # app_config = AppConfig.objects.get(id=app_config_id)
    
    url = f'{settings.MAIN_LOAD_URL_READ_CUSTOMERS}'
    customers_to_save = []
    customers_to_get = []
    
    logger.info(f"Fetching customers from Zoho Books API at {url} with params {params}")
        ...
        try:
            ...
            if response.status_code == 401:  
                ...
                if customers.get('results', []):
                    customers_to_get.extend(customers['results'])
                if 'next' in customers and customers['next']:
                    params['page'] += 1 
                else:
                    break
