...
from api_zoho_purchase_orders.models import ZohoPurchaseOrder
...
def view_purchase_order(request, purchase_order_id):
    ...
    if valid_token:
        ...
        zoho_purchase_order = ZohoPurchaseOrder.objects.get(purchaseorder_id=purchase_order_id)
        ...
        for zoho_item in all_items_data: 
            for item in zoho_purchase_order.line_items:
                if item.get('item_id') == zoho_item['item_id']:
                    item['qb_list_id'] = zoho_item['qb_list_id']

            for item in zoho_purchase_order.items_unmatched:
...
def list_purchase_orders(request):
    sales_order_list_query = ZohoPurchaseOrder.objects.all().order_by('-purchaseorder_number')
