...
const SalesOrdersDetails = () => {
  ...
  const filterSalesOrders = (flt, searchTerm) => {
    ...
    return allSalesOrders.filter((so) => {
      ...
      const matchesSearch = !q || (f.salesorder_number || '').toLowerCase().includes(q) || (f.date || '').toLowerCase().includes(q);
  ...
  const handleDeleteSalesOrder = useCallback(async (inv) => {
    if (!inv) return;
  ...
  const getBackgroundColor = (fi) =>
    salesOrder && fi.fields.salesorder_id === salesOrder.salesorder_id ? 'bg-gray-300' : '';

  const handleChangePage = (newPage) => setPage(newPage);
  const handleChangeRowsPerPage = (rows) => { setRowsPerPage(rows); setPage(0); };

  useEffect(() => {
    const idx = filteredSalesOrders?.findIndex((i) => salesOrder ? i?.fields?.salesorder_id === salesOrder?.salesorder_id : false);
    setCurrentIndexInList(idx ?? -1);
  }, [salesOrder, filteredSalesOrders]);

  useEffect(() => { setSelectedId(salesOrder?.salesorder_id ?? null); }, [salesOrder]);
  ...
  const filterConfig = {
    ...
    searchPlaceholder: 'Search Sales Order', handleSearchSelectChange,
  };

  const navItems = [
    { label: 'Resync Sales Order', icon: <RefreshCw size={16} className="mr-1" />, visible: true },
    { label: 'Delete Sales Order', icon: <Trash2 size={16} className="mr-1" />, onClick: () => handleDeleteSalesOrder(salesOrder), visible: true },
  ];

  if (loading) return <AlertLoading message="Sales Order Details" />;
  ...
  return (
    <div className="w-full py-2">
      {!salesOrder ? (
        <div>
          <Alert><AlertDescription>No sales order found.</AlertDescription></Alert>
          <div className="mt-3">
        ...
        <div className="flex flex-col md:flex-row gap-0 items-stretch">
              ...
              <Table>
                <TableBody>
                  {filteredSalesOrders && filteredSalesOrders.length > 0 ? (
                      ...
                      return (
                        ...
                        >
                          <TableCell className="py-3">
                            <b>{fi.fields.salesorder_number}</b><br />
                    ...
                    <TableRow><TableCell>No sales orders found.</TableCell></TableRow>
                  )}
          ...
          <div className="flex-1 min-w-0 flex flex-col">
            <div className="flex items-center justify-between mb-2 ml-1 flex-shrink-0">
              <h2 className="text-lg font-bold uppercase text-[#212529]">{salesOrder.salesorder_number}</h2>
              <div className="flex gap-1">
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button variant="ghost" size="icon" onClick={() => {
                        const prev = filteredSalesOrders[currentIndexInList - 1] || filteredSalesOrders[filteredSalesOrders.length - 1];
                        if (prev) handleViewSalesOrder(prev.fields.salesorder_id);
                ...
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button variant="ghost" size="icon" onClick={() => {
                        const next = filteredSalesOrders[currentIndexInList + 1] || filteredSalesOrders[0];
                        if (next) handleViewSalesOrder(next.fields.salesorder_id);
                ...
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button variant="ghost" size="icon" onClick={() => navigate('/integration/list_sales_orders')}><X size={18} /></Button>
                    </TooltipTrigger>
                ...
                <TableBody>
                  ...
                  <TableRow>
                    ...
                    <TableCell className="border-none">
                      <div
                        className={`flex items-center justify-between rounded px-2 py-1 cursor-pointer ${salesOrder.qb_customer_list_id ? 'bg-green-50' : 'bg-amber-50'}`}
                        onClick={() => handleViewCustomer(salesOrder.customer_id)}
                        onMouseOver={() => !salesOrder.qb_customer_list_id && setHovered(true)}
                        onMouseOut={() => setHovered(false)}
                      >
                        <b className={hovered && !salesOrder.qb_customer_list_id ? 'font-bold text-red-600' : ''}>{salesOrder.customer_name}</b>
                        <SmallBadge matched={!!salesOrder.qb_customer_list_id} />
                      </div>
                    </TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell className="border-none font-medium">Date</TableCell>
                    <TableCell className="border-none"><b>{salesOrder.date}</b></TableCell>
                  </TableRow>
                  <TableRow>
                    <TableCell className="border-none font-medium">Total Amount</TableCell>
                    <TableCell className="border-none"><b>$ {salesOrder.total}</b></TableCell>
                  </TableRow>
                  {salesOrder.last_sync_date && (
                    <TableRow>
                      <TableCell className="border-none font-medium">Last Sync Date</TableCell>
                      <TableCell className="border-none"><b>{salesOrder.last_sync_date}</b></TableCell>
                    </TableRow>
                  )}
                  <TableRow>
                    <TableCell className="border-none font-medium">Number of attempts to sync</TableCell>
                    <TableCell className="border-none"><b>{salesOrder.number_of_times_synced}</b></TableCell>
                  </TableRow>
                      ...
                      {salesOrder.line_items.length > 0 ? (
                        <div className="border border-border rounded overflow-auto max-h-[50vh]">
                          <Table>
                            ...
                            <TableBody>
                              {[...salesOrder.line_items].sort((a, b) => {
                  ...
                  {(salesOrder.inserted_in_qb || salesOrder.items_unmatched.length > 0) && (
                    <TableRow>
                      <TableCell className="border-none font-medium"><b>{!salesOrder.inserted_in_qb ? 'ERRORS ' : ''}</b>Sync Items</TableCell>
                      <TableCell className="border-none">
                                  ...
                                  <TableRow key={idx}>
                                    ...
                                    <TableCell>
                                      <Alert variant="destructive" className="text-xs py-1">
                                        <AlertDescription><b>{it.reason}</b>{it.qb_list_id && <><br />You can run QBWC Sales Orders again to update this item.</>}</AlertDescription>
                                      </Alert>
                                    </TableCell>
                                    <TableCell>
                                      {!it.qb_list_id ? (
                  ...
                  {(salesOrder.inserted_in_qb || salesOrder.customer_unmatched.length > 0) && (
                    <TableRow>
                      <TableCell className="border-none font-medium"><b>{!salesOrder.inserted_in_qb ? 'ERRORS ' : ''}</b>Sync Customer</TableCell>
                      <TableCell className="border-none">
                              ...
                              <TableBody>
                                {salesOrder.customer_unmatched.map((cu, idx) => (
                                  <TableRow key={idx}>
                                    ...
                                    <TableCell>
                                      <Alert variant="destructive" className="text-xs py-1">
                                        <AlertDescription><b>{cu.reason}</b>{cu.qb_list_id && <><br />You can run QBWC Sales Orders again to update this customer.</>}</AlertDescription>
                                      </Alert>
                                    </TableCell>
                                    <TableCell>
                                      {!cu.qb_list_id ? (
